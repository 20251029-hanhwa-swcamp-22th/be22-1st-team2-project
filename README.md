...
## 2. 팀 소개
    - 강성훈(팀장) : 프로젝트 팀장(+발표, 자료정리, 요구사항 정리)
...
## 테스트케이스
    - 시나리오별 SQL 및 실행 결과
    1. CS-001 : 회원가입
    2. CS-002 : 로그인, 예약정보 확인
    1. CS-003,004, 005 : 회원정보 / 보안정보 / 프로필 수정
    1. CS-006 : 회원탈퇴
    1. MS-003 : 전체 문의
    1. PS-001 : 서비스 제공사 관련
    
## 프로젝트 회고
- 강성훈 : 이번 프로젝트를 통해 DB의 전체적인 흐름을 파악하게 되었고, 개인간의 역량도 중요하지만 프로젝트는 팀원간의 협동심이 중요하다고 생각했다. 적재적소한 역할분담으로 정확하고 신속하게 체계적으로 프로젝트를 진행하는 것이 그것이 진정한 팀 프로젝트라고 느꼈다.
- 김신우 : 오랜만에 요구사항 분석부터 DB 설계 및 구현까지 해보는 의미있는 프로젝트였다. 팀원들간의 소통이 원활하고 의사소통이 활발하여 큰 어려움 없이 프로젝트를 완료할 수 있었다. 앞으로의 팀 프로젝트도 기대가 된다.
- 정병진
  - 느낀점 : 프로젝트 하면서 진짜 많이 생각해야된다라는 걸 뼈저리게 느꼈습니다..데이터베이스 전반적인 구조를 파악할 수 있는 좋은 프로젝트였고, 처음하는 협업 프로젝트를 진행하면서 팀원들에게 많은 도움을 받아 공부에 도움이 많이 됐습니다. 빨리 자바랑 연결해서 활용해보고 싶습니다.
